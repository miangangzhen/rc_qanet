...
from my_code.qanet_layers_bfloat16 import initializer, regularizer, residual_block, highway, conv, mask_logits, trilinear, total_params, \
    position_embedding
...
class QANET_Model(object):
    """
    param:
    --batch_size 16 --learning_rate 1e-3 --optim adam --decay 0.9999 --weight_decay 1e-5 --max_norm_grad 5.0 --dropout 0.0 --head_size 1 --hidden_size 64 --epochs 20 --gpu 0
    """
    def __init__(self, vocab, config, demo=False):
        ...
        with tf.variable_scope("qanet", dtype=tf.float16):
            self._build_graph()
    ...
    def _setup_placeholders(self):
        ...
        self.c = tf.reshape(self.c, [self.config.batch_size * self.max_p_num, self.config.max_p_len])
        self.q = tf.reshape(self.q, [self.config.batch_size * self.max_p_num, self.config.max_q_len])
        ...
        self.dropout = tf.placeholder(tf.float16, name="dropout")
    ...
    def _compute_loss(self):
        def focal_loss(logits, labels, weights=None, alpha=0.25, gamma=2):
            ...
            cross_ent = - alpha * (pos_p_sub ** gamma) * tf.log(tf.clip_by_value(logits, 1e-4, 1.0)) \
                        - (1 - alpha) * (neg_p_sub ** gamma) * tf.log(tf.clip_by_value(1.0 - logits, 1e-4, 1.0))
            return tf.reduce_sum(cross_ent, 1)

        start_label = tf.one_hot(self.start_label, tf.shape(self.logits1)[1], axis=1, dtype=tf.float16)
        end_label = tf.one_hot(self.end_label, tf.shape(self.logits2)[1], axis=1, dtype=tf.float16)
        ...
        if self.config.loss_type == 'cross_entropy':
            ...
            start_loss = focal_loss(tf.nn.softmax(self.logits1, -1), start_label, alpha=0.95, gamma=0.5)
            end_loss = focal_loss(tf.nn.softmax(self.logits2, -1), end_label, alpha=0.95, gamma=0.5)
            self.loss = tf.reduce_mean(start_loss + end_loss)
    ...
    def _create_train_op(self):
        # self.lr = tf.minimum(tf.cast(self.learning_rate, dtype=tf.float16), self.learning_rate / tf.log(tf.constant(999.0, dtype=tf.float16)) * tf.log(tf.cast(self.global_step, tf.float16) + tf.constant(1.0, dtype=tf.float16)))
