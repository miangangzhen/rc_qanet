...
def position_embedding(inputs, position_dim):
    ...
    pos_j = tf.cast(pos_j, dtype=tf.float16)
    pos_j = tf.expand_dims(pos_j, 0)
    pos_i = tf.range(tf.cast(seq_len, dtype=tf.int32))
    pos_i = tf.cast(pos_i, dtype=tf.float16)
    ...
    outputs = tf.expand_dims(pos_ij, 0) \
              + tf.zeros((batch_size, seq_len, position_dim), dtype=tf.float16)
    return outputs
...
def layer_norm(x, filters=None, epsilon=1e-4, scope=None, reuse=None):
...
def layer_dropout(inputs, residual, dropout):
    pred = tf.random_uniform([], dtype=tf.float16) < dropout
...
def add_timing_signal_1d(x, min_timescale=1.0, max_timescale=1.0e4):
    ...
    return x + tf.cast(signal, dtype=tf.float16)
