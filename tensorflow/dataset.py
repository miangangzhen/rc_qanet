...
"""
"""
优化了数据加载内存占用，而原版dataset放在.back文件里了。改动了_load_dataset
"""
...
class BRCDataset(object):
    ...
    def __init__(self, max_p_num, max_p_len, max_q_len,
        ...
        if train_files:
            for train_file in tf.gfile.Glob(train_files):
                self.train_set += self._load_dataset(train_file, train=True, limit=limit)
    ...
    def _load_dataset(self, data_path, train=False, limit=None):
        """
        Loads the dataset
        Args:
        ...
        with open(data_path, encoding="utf-8") as fin:
            ...
            for lidx, line in enumerate(fin):
                ...
                if 'answer_docs' in sample:
                    data['answer_passages'] = sample['answer_docs']

                data['question_tokens'] = sample['segmented_question']
                ...
                if is_solo_answer:
                    if "。" == "".join(data["passages"][data["answer_passages"][0]]["passage_tokens"][
    ...
    def gen_mini_batches_for_qanet(self, set_name, batch_size, pad_id, shuffle=True):
        """
                Generate data batches for a specific dataset (train/dev/test)
                Args:
                    set_name: train/dev/test to indicate the set
        ...
        for batch_start in np.arange(0, data_size, batch_size):
            ...
            yield self._one_mini_batch_for_qanet(data, batch_indices, pad_id)

    def _one_mini_batch_for_qanet(self, data, indices, pad_id):
        batch_data = {'raw_data': [data[i] for i in indices],
                      'question_token_ids': [],
                      'question_length': [],
                      'passage_token_ids': [],
        ...
        for sidx, sample in enumerate(batch_data['raw_data']):
            for pidx in range(max_passage_num):
                if pidx < len(sample['passages']):
                    ...
                    batch_data['passage_length'].append(0)
        # 这里不同
        batch_data, padded_p_len, padded_q_len = self._dynamic_padding_for_qanet(batch_data, pad_id)
        for sample in batch_data['raw_data']:
            if 'answer_passages' in sample and len(sample['answer_passages']):
                gold_passage_offset = padded_p_len * sample['answer_passages'][0]
                batch_data['start_id'].append(gold_passage_offset + sample['answer_spans'][0][0])
    ...
    def _dynamic_padding_for_qanet(self, batch_data, pad_id):
