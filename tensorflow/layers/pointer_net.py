...
class PointerNetLSTMCell(tc.rnn.LSTMCell):
    ...
    def __init__(self, num_units, context_to_point):
        ...
        # context_to_point = [batch_size, sequence_length*max_passage_num, hidden_size*2]
        self.context_to_point = context_to_point
        # fc_context = [batch_size, sequence_length*max_passage_num, hidden_size]
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__):
            # U [batch_size, sequence_length*max_passage_num, hidden_size]
            U = tf.tanh(self.fc_context
                        + tf.expand_dims(tc.layers.fully_connected(m_prev,
                                                                   num_outputs=self._num_units,
                                                                   activation_fn=None),
                                         1))
            # logits [batch_size, sequence_length*max_passage_num, 1]
            logits = tc.layers.fully_connected(U, num_outputs=1, activation_fn=None)
            # scores [batch_size, sequence_length*max_passage_num, 1]
            scores = tf.nn.softmax(logits, 1)
            # attended_context [batch_size, 1]
...
class PointerNetDecoder(object):
    ...
    def decode(self, passage_vectors, question_vectors, init_with_question=True):
        ...
        Args:
            passage_vectors: the encoded passage vectors [batch_size, sequence_length*max_passage_num, hidden_size*2]
            question_vectors: the encoded question vectors [batch_size, sequence_length, hidden_size*2]
